...
class PaytmController extends DepositBaseController
{
    ...
    public function store(Request $request)
    {
     ...
     $supported_currency = ['INR'];
            if(!in_array($curr->name,$supported_currency)){
                Toastr::error('Invalid Currency For Paytm Payment.', 'Error');
    ...
    public function notify( Request $request ) {
        ...
        $deposit = Deposit::where('user_id','=',Session::get('item_number'))->orderBy('id','DESC')->first();
        ...
        if ($deposit->status == 1) {
          ...
          $transaction->save();
            ...
            Toastr::success('Deposit Successfully Done.', 'Success');
     ...
     Toastr::error('Deposit Failed.', 'Error');
	return redirect()->back();
    }
}
